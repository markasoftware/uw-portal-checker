...
(defconst uw--login-failed-text "UW NetId sign-in - Uncaught Exception"
  "The text that will appear in a failed login action page")
(defconst uw--idp-domain "https://idp.u.washington.edu"
  "The domain of the login form action")
(defconst uw--my-sso-link "https://my.uw.edu/saml/sso"
  "The SSO post action.")
(defconst uw--application-status-link "https://my.uw.edu/out?u=https://sdb.admin.uw.edu/admissions/uwnetid/appstatus.asp"
  "The page which is updated when decisions are announced.")
(defconst uw--undecided-text "This message will not change unless additional information is required or until a final admission decision is made."
...
(defun uw--logged-in-p nil
  "Determine if the current buffer is the result of a successful NetID login"
  (save-excursion
    (save-match-data
      (goto-char (point-min))
      (not (search-forward uw--login-failed-text nil t)))))
  ...
  (let ((uw--login-action (uw--pre-login)))
    (with-current-buffer
        (uw--submit-urlencoded-form
         ...
         uw--login-action)
      (uw--follow-sso-redirects)
      (unless (uw--logged-in-p)
        (error "UW NetID login login failed, probably incorrrect username or password")))))

(defun uw--status-page nil
  "Get the status page. Logs in first."
  (uw--login)
